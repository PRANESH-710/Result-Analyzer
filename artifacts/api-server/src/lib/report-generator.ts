...
export function generateMarkdownReport(analysis: AnalysisResult, showStudentIds: boolean): string {
  ...
  report.push('# 📊 Student Examination Results Analysis Report');
  report.push('');
  ...
  report.push(`**Pass Criteria:** ${analysis.passPercentage}% minimum per subject`);
  report.push('');
  ...
  report.push('');
  report.push('| Subject | Students | Pass Rate | Avg Score | Highest | Topper |');
  report.push('|---------|----------|-----------|-----------|---------|--------|');
  ...
  for (const [subject, stats] of Object.entries(analysis.subjectWiseStats)) {
    ...
    report.push(`| ${subject} | ${stats.totalCount} | ${stats.passRate.toFixed(dp)}% | ${stats.averageScore.toFixed(dp)} | ${stats.highestScore.toFixed(dp)} | ${topperName} |`);
  }
  report.push('');
  ...
  if (analysis.anomalies.length > 0) {
    report.push('## ⚠️ Anomalies and Concerns');
    report.push('');
  ...
  report.push('---');
  report.push('*Generated by the Academic Performance Analyzer*');
  report.push('');
